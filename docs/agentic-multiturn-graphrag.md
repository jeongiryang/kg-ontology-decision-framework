...
한 턴의 계획 도구는 최대 4개, 하위 질문은 최대 3개, 대화에서 전달하는 course code는
최대 20개다. 동일 질문의 재조회는 제한하며, 첫 조회가 `NOT_FOUND` 또는 `UNRESOLVED`인
경우에만 같은 주제의 좁은 후속 조회 한 번을 허용한다. LLM이 만든 도구 인자는 직접
실행하지 않고 오케스트레이터가 검증된 question/context/profile에서 다시 구성한다.
...
- 복잡한 목록과 원문 규칙은 자연어 재작성을 하지 않고 canonical 문장을 유지한다.
