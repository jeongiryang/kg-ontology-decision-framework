...
문제를 해결한다. 최종 PM 결정에 따라 처리 중 그래프 애니메이션은 제외하고, 처리 완료
뒤 승인된 스키마 요약, canonical Cypher, 질의 구조와 ClaimValidator provenance만
정적으로 표시한다.
...
2. `SCHEMA_SELECTION`은 선택된 label·relationship badge만 만들고 Cypher를 노출하지
   않는다.
3. canonical Cypher와 질의 경로는 같은 후보가 정적 검증과 EXPLAIN을 모두 통과한 뒤에만
   표시한다.
4. ClaimValidator가 승인한 정확한 Fact–Evidence 쌍만 provenance edge로 만든다.
5. 처리 중 화면은 실제 callback 텍스트 타임라인만 사용한다. node fade, edge pulse,
   callback별 graph redraw와 실시간 탐색 상태는 최종 범위에서 제거한다.

## 4. 수행 작업

- 답변 완료 화면에 `선택 스키마`, `승인 Cypher`, `조회 그래프` 탭을 추가했다.
- 최종 승인 데이터가 없는 탭은 비활성화한다.
- 단계 disclosure는 안전한 상세 정보가 있는 단계에만 생성하고 해당 탭 이동 링크를
  제공한다.
- EXPLAIN 승인 query structure와 직접 Evidence provenance를 정적으로 렌더링한다.
...
- 초기 구현의 node fade, query/provenance pulse와 callback별 graph redraw는 최종 결정에
  따라 제거했다.
...
| 검증 항목 | 명령 또는 방법 | 결과 |
|---|---|---|
| 관련 최소 테스트 | `uv run --no-sync pytest -q tests/test_evidence_chat.py tests/test_dynamic_query_safety.py` | 55 passed, 39 subtests passed |
...
| 상세 inspection | 임시 `KG_CHAT_SHOW_QUERY_DETAILS=true` 서버 | 정적 query 5 node와 provenance 18 node/9 edge만 확인, 승인 Cypher event가 result보다 먼저 도착 |
| Neo4j 전후 개수 | read-only count | node 1,518 / relationship 3,260 / Evidence 511로 동일 |
...
- projection은 처리 완료 뒤 승인 산출물을 정적으로 표시하며 Neo4j 내부 token 이동
  이벤트나 실시간 탐색 애니메이션이 아니다.
